...
tic;
fprintf('Test Output... ');
fp_predict = fopen(sprintf('%s/Test.predict1.%s.csv', DATA_DIRECTORY, preprocessing_method), 'w');
fprintf(fp_predict, 'author_id,paper_id,features\n');
dlmwrite(sprintf('%s/Test.predict1.%s.csv', DATA_DIRECTORY, preprocessing_method), [test_data(:,1:(size(test_data,2)-1)) y3], 'delimiter', ',', 'precision', 10, '-append');
fprintf('%f s\n', toc);
fclose(fp_predict);

%% Unfold result and calculate MAP score
system(sprintf('python ../script/result.py ../data/Test.predict1.%s.csv %d', preprocessing_method, postprocessing_critical));
end
